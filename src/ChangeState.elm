...
changeState model =
    ...
    if arriveExit model model.player then 
        case model.state of
            One -> 
                { model | map = initMapDiscoverI, state = Break, prestate = One, player = playerDiscoverI, time = 0}
            DiscoverI ->
                { model | map = initMap2, state = Break, prestate = DiscoverI, player = player2, time = 0}
            Two ->
                if arriveExit model model.speedAI then
                    { model | map = initMapDiscoverII, state = Break, prestate = Two, player = playerDiscoverIILose, time = 0}
                else
                    { model | map = initMapDiscoverII, state = Break, prestate = Two, player = playerDiscoverIIWin, time = 0}
            DiscoverII ->
                { model | map = initMap3, state = Break, prestate = DiscoverII, player = player3, time = 0}
            Three ->
                { model | map = initMap1, state = Story6_1, prestate = Three, player = initPlayer1, time = 0}
            _ ->
                model
    else
        model
        ...
        case model.state of
            LOGO ->
                if model.player.teachtextstate == 0 then
                    { model | state = Model.Start, prestate = LOGO, cgtime = 500}
                else
                    { model | state = End, prestate = LOGO, cgtime = 500}
        -- One
            Story1_1 ->
